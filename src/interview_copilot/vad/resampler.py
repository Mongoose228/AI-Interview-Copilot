...
class Resampler:
    ...
    def process(self, audio_data: np.ndarray, in_channels: int = 2) -> np.ndarray:
        """
        Process audio chunk (np.ndarray), mixdown to mono if needed, and resample.
        Returns float32 numpy array.
        """
        samples = audio_data

        # If stereo, mix to mono
        if in_channels == 2 and samples.ndim == 2 and samples.shape[1] == 2:
            samples = samples.mean(axis=1)
        elif in_channels > 2 and samples.ndim == 2:
            samples = samples.mean(axis=1)
        elif samples.ndim == 2:
            # Flatten if unexpected shape
